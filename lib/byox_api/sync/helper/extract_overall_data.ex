defmodule ByoxApi.Sync.Helper.ExtractOverallData do
  ...
  defp extract_category_text(category_tag) do
    case Regex.run(~r{<a href="#[^>]+">(?<category>[^<]+)</a>}, category_tag, capture: [:category]) do
      [category] -> category
      _ -> nil
    end
  end
  ...
  defp parse_tag(tag) do
    case Regex.run(~r{<a href="(?<url>[^"]+)"><strong>(?<language>[^<]+)</strong>: <em>(?<title>[^<]+)</em></a>}, tag, capture: [:url, :language, :title]) do
