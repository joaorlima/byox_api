defmodule ByoxApi.Sync.Helper.ExtractTutorials do
  ...
  def map_and_create(tutorials_data) do
    data_split_by_languages = tutorials_data
    |> Enum.reject(&is_nil/1)
    |> Enum.map(&data_split_by_languages/1)

    mapped_data = data_split_by_languages
  ...
  defp create_tutorial_struct(mapped_data) do
    ...
    |> Enum.map(fn tutorial_data ->
      ...
      %{title: tutorial_data.title, url: tutorial_data.url, topic_id: topic.id, language_id: language.id}
